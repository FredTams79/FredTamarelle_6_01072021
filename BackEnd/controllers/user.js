  ...
  if (!schema.validate(req.body.password)) {
    ...
    bcrypt
      ...
      .then((hash) => {
        ...
        const user = new User({
          email: maskData.maskEmail2(req.body.email, emailMask2Options), //si utilisation de maskdata pour masquer l'email de l'utilisateur : email: maskData.maskEmail2(req.body.email, emailMask2Options),
          password: hash,
        });
...
exports.login = (req, res, next) => {
  ...
  User.findOne({
    email: maskData.maskEmail2(req.body.email, emailMask2Options), //si utilisation de maskdata : email: maskData.maskEmail2(req.body.email, emailMask2Options),
  })
    .then((user) => {
